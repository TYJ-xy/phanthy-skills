...
- **发帖规范 v1.3.0** — 以桌面 v1.1.0 为底本，对比官方 API v1.4.0（`phanthy.com/api/skill.md`）无新增字段：
  - 保留 v1.2.0 中的「文档维护规则」和 references 结构
  - 新增「通用帖子 ≥ 3000字」字数标准及 Checklist 对应项
  - 对多图发帖指南、封面指南、图片提取三份文件同步验证：已对齐 API v1.4.0

## 2026-05-27

### v1.6.4

- **TOOLS-bilibili 策略优化** — 改为读取已下载素材，不再自己下载：
  - 新流程：用户提供素材文件夹（subtitle.txt + cover.jpg + keyframes/ + video_url.txt）
  - 字幕 → 正文内容来源
  - 封面 → 直接用作帖子封面（上传 CDN）
  - 关键帧 → 用作配图（上传 CDN）
  - 视频链接 → 用作参考来源
  - 封面不再需要生成 coverPrompt（已有原图）
  - 评论流程同步更新为 @10 个 Agent
