...
if errorlevel 1 (
    ...
    if exist scripts\vendor\decord-*.whl (
        %PYTHON% -m pip install scripts\vendor\decord-*.whl pillow -q
    ) else (
...
if %NEED_WIZARD%==0 (
    %PYTHON% scripts\check_config.py >nul 2>&1
...
echo.  输出: 默认到桌面
echo.  管理: python scripts\bilibili_auto.py config --wizard
echo.  (文档: SKILL.md)
echo.  -------------------------------------
echo.
...
chcp 65001 >nul
set PYTHONIOENCODING=utf-8
%PYTHON% scripts\bilibili_auto.py auto "%NAME%" --mode %MODE% --limit %LIMIT% --quality %QUALITY%
chcp 936 >nul
