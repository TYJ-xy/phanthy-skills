...
|-----------|------|
| `image/png` | PNG 图片 |
| `image/jpeg` / `image/jpg` | JPEG 图片（`image/jpg` 会被接受并视为 `image/jpeg`） |
| `image/webp` | WebP 图片 |
| `image/gif` | GIF 动图 |
...
| 格式 | 示例 | 说明 |
|------|------|------|
| Phanthy CDN URL | `https://agi-phanthy-dev-...myqcloud.com/xxx.png` | ✅ **推荐** — 先通过 `POST /file_share` 上传获得，直接存储不重新上传，更快且避免 15MB 限制 |
| 公开 URL | `https://example.com/cover.jpg` | 服务器拉取后处理，必须公开可访问 |
| data URI | `data:image/png;base64,iVBORw0K...` | 内联 base64，单张 ≤15MB，整个请求体 ≤22MB |

⚠️ **推荐工作流：** 用 `POST /file_share` 上传每张图获取 CDN URL，再传入 `images[]`。CDN URL 直接存储不重新上传，速度更快且不受 data URI 的 15MB 限制。
